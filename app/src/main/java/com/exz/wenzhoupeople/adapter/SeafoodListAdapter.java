...
public class SeafoodListAdapter extends BaseQuickAdapter<SeafoodListEntity, BaseViewHolder> {
    ...
    public SeafoodListAdapter() {
        super(R.layout.adapter_seafood_list, null);
    }
    ...
    protected void convert(BaseViewHolder helper, SeafoodListEntity item) {
        ...
        try {
            if(!TextUtils.isEmpty(item.getTitle()))
                subTitle.setText(URLDecoder.decode(item.getSubTitle(),"UTF-8"));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
